...
var DAO = function(){
  ...
  */
  ...
  /**
  The tag label is the combination between a tag and a label.
  A label can be added to multiple tags. So the combinations are kept seperately.
  The method removes the entry from this table which contains the entries.
  @param {integer} tagID
  @param {integer} labelID
  */
  this.removeTagLabel=function(labelID,tagID,callback){
    var temp_error = null;
    var removed = false;
    this.pool.getConnection(function(err, connection) {
      if(err) throw err;
      connection.query('DELETE FROM TagLabel WHERE labelID = ? and tagID = ?', [labelID,tagID], function(error, results, fields) {
        /* This is an internal server error, this should never happen */
        if(error) {
          console.log("[DAO] Unable to removeTagLabel(): " + error.code);
          temp_error = new Error("Tag/label combination wasn't removed from the database."+error.code);
        } else if(results.affectedRows == 1) {
          removed = true;
        }
